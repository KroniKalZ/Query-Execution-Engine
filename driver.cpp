...
void runFileReaderQuery() {
  FileReader fr("smallData/department.data");
  executeQuery(&fr);
}

void runQuery() {
  Select op1("smallData/student.data", "S_dept_name","=","Comp. Sci.");
  Select op2("smallData/instructor.data","I_dept_name","=","Comp. Sci.");
  Join   op3(&op1, &op2, "S_dept_name","I_dept_name");
  executeQuery(&op3);
}
